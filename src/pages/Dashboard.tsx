...
import { CreditCard, CreditCardMonthly, Investment, InvestmentMonthly, CarSavings, Service, Debt, Expense, DollarRate } from '../types';
...
export default function DashboardPage({ data, month }: DashboardPageProps) {
  ...
  const { items: carSavings } = useTable<CarSavings>('carSavings');
  ...
  const currentMonthCS = carSavings.filter((cs) => cs.month === month);
  const currentMonthSvc = services.filter((svc) => svc.month === month);
  const currentMonthDebt = debts.filter((d) => d.month === month);
  ...
  const totalCS = currentMonthCS.reduce((sum, cs) => sum + cs.amountPesos, 0);
  const totalSvc = currentMonthSvc.reduce((sum, svc) => sum + svc.amountPesos, 0);
  const totalDebt = currentMonthDebt.reduce((sum, d) => sum + d.amountPesos, 0);
  const totalExp = currentMonthExp.reduce((sum, exp) => sum + exp.amountPesos, 0);
  const grandTotal = totalCC + totalInv + totalCS + totalSvc + totalDebt + totalExp;
  ...
  return (
    <div className="page">
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          <div className="stat-card-label">Car Savings</div>
          <div className="stat-card-value">${totalCS.toLocaleString('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}</div>
          <div className="stat-card-change">{currentMonthCS.length} records</div>
        </div>
